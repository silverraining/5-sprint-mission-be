...
app.listen(process.env.PORT || 5005, () => console.log("Server Started"));
...
productsRouter.get("/:id/comments", async (req, res) => {
  ...
  try {
    const comment = await prisma.product_Comment.findMany({
      where: {
        productId: productId, // Look up the comment by commentId
      },
    });
    if (!comment) {
      return res.status(404).json({ error: "Comment not found" });
...
productsRouter.get("/:id/comments/:commentId", async (req, res) => {
  ...
  try {
    const comment = await prisma.product_Comment.findUnique({
      where: {
        id: commentId, // Query by commentId, which is unique
      },
    });

    // Ensure the comment belongs to the correct productId
    if (!comment || comment.productId !== productId) {
      return res
        .status(404)
        .json({ error: "Comment not found for this product" });
    }

    res.send(comment); // Return the specific comment
  } catch (error) {
